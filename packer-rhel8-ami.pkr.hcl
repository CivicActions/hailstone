...
source "amazon-ebs" "autogenerated_1" {
  ...
  ami_name                    = "RHEL8-hardened-${legacy_isotime("2006-01-02-1504")}"
...
build {
  ...
  provisioner "file" {
    ...
    source      = "./ssg-rhel8-ds-tailoring.xml"
  }

  provisioner "shell" {
    expect_disconnect = "true"
    inline            = ["echo '****   Updating kernel   ****'", "sudo dnf -y update kernel kernel-tools*", "sudo systemctl reboot"]
  }

  provisioner "shell" {
    environment_vars = ["bucket=${var.bucket_name}", "ami_name=RHEL8-hardened-${legacy_isotime("2006-01-02-1504")}"]
    execute_command  = "{{ .Vars }} sudo -E -sS bash '{{ .Path }}'"
    pause_before     = "40s"
    script           = "setup-rhel8.sh"
    valid_exit_codes = [0]
  }
